...
import Encuesta_Dengue_1 from './Encuesta/Encuesta_Dengue_1';
import { saveInformationToDatabase } from '~/Servicios/postresql';
import { cleanIcon } from '~/utils/utils';

const Menu_Dengue_4 = addKeyword(EVENTS.ACTION)
    ...
    .addAnswer('¿Cuantos años cumplidos tienes?',
        ...
        async (ctx, { state, gotoFlow, fallBack }) => {
            ...
            if (edad < 1 || edad > 100) {
                ...
                if (myState.genero === '1') {
                    stop(ctx);
                    return gotoFlow(Encuesta_Dengue_1)
                }
            }
        })
